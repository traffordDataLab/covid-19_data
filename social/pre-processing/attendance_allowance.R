...
# Source: DWP
# URL: https://stat-xplore.dwp.gov.uk/webapi/metadata/AA_In_Payment_New/AA_In_Payment_New.html
...
query <- list(database = unbox("str:database:AA_In_Payment_New"),
              measures = "str:count:AA_In_Payment_New:V_F_AA_In_Payment_New",
              dimensions = c("str:field:AA_In_Payment_New:V_F_AA_In_Payment_New:COA_CODE",
                             "str:field:AA_In_Payment_New:F_AA_QTR_New:DATE_NAME") %>% matrix(),
              recodes = list(
                `str:field:AA_In_Payment_New:V_F_AA_In_Payment_New:COA_CODE` = list(
                  map = as.list(paste0("str:value:AA_In_Payment_New:V_F_AA_In_Payment_New:COA_CODE:V_C_MASTERGEOG11_MSOA_TO_LA:E0", seq(2001259, 2001286, 1)))),
                `str:field:AA_In_Payment_New:F_AA_QTR_New:DATE_NAME` = list(
                  map = as.list(paste0("str:value:AA_In_Payment_New:F_AA_QTR_New:DATE_NAME:C_AA_QTR_New:",c(201805,201808,201811,201902,201905,201908,201911))))
              )) %>% toJSON()
...
df <- as.data.frame.table(values, stringsAsFactors = FALSE) %>%
  ...
  mutate(indicator="Attendance Allowance (cases in payment)", 
...
write_csv(df, "attendance_allowance.csv")
