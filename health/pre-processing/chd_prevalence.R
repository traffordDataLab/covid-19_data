...
df <- read_xlsx(tmp,sheet = 4) %>%
  ...
  mutate(indicator="Coronary Heart Disease prevalence",period="2018",measure="percentage",unit="persons", value=round(`Coronary Heart Disease`*100,2)) %>%
  select(area_code="Row Labels", area_name="MSOA", indicator, period, measure, unit, value) %>%
  arrange(area_code)

write_csv(df, "chd_prevalence.csv") 
