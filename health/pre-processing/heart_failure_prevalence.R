...
df <- read_xlsx(tmp,sheet = 4) %>%
  ...
  mutate(indicator="Heart Failure prevalence",period="2018",measure="percentage",unit="persons", value=round(`Heart Failure`*100,2)) %>%
  select(area_code="Row Labels", area_name="MSOA", indicator, period, measure, unit, value) %>%
  arrange(area_code)

write_csv(df, "heart_failure_prevalence.csv") 
